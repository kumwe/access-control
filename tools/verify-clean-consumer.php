...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-access-control-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-access-control',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-access-control.zip';
if (!is_file($archive)) {
...
$installed = $consumer . '/vendor/kumwe/access-control';
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Access\\Tests\\')
    ) {
